...
class PipelineState(BaseModel):
    ...
    def advance_to(self, stage: PipelineStage) -> None:
        """Advance the pipeline to the given stage. The previous current_stage
        is appended to completed_stages."""
    ...
    def mark_failed(self, stage: PipelineStage, error_msg: str) -> None:
        """Record a stage failure with an error message."""
        self.failed_stage = stage
        self.error_message = error_msg

    def is_complete(self) -> bool:
        """Return True if the pipeline has reached and completed the final stage."""
        return (
    ...
    def get_stage_status(self, stage: PipelineStage) -> str:
        """Return the status of a given stage: 'completed', 'current', 'failed', or 'pending'."""
